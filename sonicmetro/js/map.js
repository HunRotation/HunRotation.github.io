...
export class SubwayMap {
    ...
    async init() {
        ...
        try {
            ...
            // Note: We do NOT set pointer-events: none on 'g' here because
            // strictly, bubbling works fine from rect -> svg. 
            // But if user wants to hover stations later, we need events on content.
            // For panning, as long as background rect catches misses, we are good.
            ...
            this.g.append("image")
                ...
                .attr("x", 0)
                .attr("y", 0)
                .style("pointer-events", "none"); // Image shouldn't blocking interactions
    ...
    setupZoom() {
        ...
        // Debug Click: Add visual marker
        this.svg.on("click.logger", (event) => {
            const [x, y] = d3.pointer(event, this.g.node());
            const normX = Math.round(x / this.scaleFactor);
            const normY = Math.round(y / this.scaleFactor);

            // Format: { name: "Station", x: 123, y: 456 },
            const logMsg = `{ name: "Station", x: ${normX}, y: ${normY} },\n`;
            console.log(logMsg);
    ...
    renderStations(stations) {
        ...
        this.g.selectAll(".station-marker")
            .data(stations)
            .join("circle") // Modern D3
            .attr("class", "station-marker")
            .attr("cx", d => d.x)
            .attr("cy", d => d.y)
            .attr("r", Math.max(R, 3)) // Min size 3
            .attr("fill", "white")
            .attr("stroke", "#0052A4")
    ...
    renderConnections() {
        ...
        const pathGroup = this.g.append("g").attr("class", "connection-paths");
        ...
        const drawPath = (rawBranch, className, color) => {
            ...
            pathGroup.append("path")
                ...
                .attr("fill", "none")
                .attr("stroke", color || "#0052A4") // Default blue if not specified, but usually overridden
    ...
    renderTrains(activeTrains) {
        ...
        const trains = this.g.selectAll(".train-marker")
